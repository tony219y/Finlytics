...
import { DeleteExpense } from "@/hooks/useExpense";
...
const RemoveExpenseDialog = ({ selectedItems, setSelectedItems, refetch }: RemoveExpenseDialogProps) => {
  const [removeOpen, setRemoveOpen] = useState<boolean>(false);
  const handleRemove = async () => {
    const response = await DeleteExpense(selectedItems);
    if (response) {
      setRemoveOpen(false);
      setSelectedItems([]);
