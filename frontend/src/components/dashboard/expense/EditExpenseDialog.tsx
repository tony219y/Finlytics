...
import { expenses, UpdateExpense } from "@/hooks/useExpense";
...
const EditExpenseDialog = ({
  ...
  const handleEdit = async (dataInput: any) => {
    const response = await UpdateExpense(selectedItems[0], dataInput);
    if (response) {
      setEditOpen(false);
      setSelectedItems([]);
      ...
      <DialogContent>
        ...
        >
          <div className="flex flex-col gap-4">
            ...
            <Label>source</Label>
            <Input {...register("category")} placeholder={data?.category} />
            {errors.category && (
